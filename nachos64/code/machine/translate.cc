...
Machine::Translate(int virtAddr, int* physAddr, int size, bool writing)
{
    ...
    // Tener un TLB o un page table
    ASSERT(tlb == NULL || pageTable == NULL);	
    ASSERT(tlb != NULL || pageTable != NULL);	

	// Calcular la virtual page number y offset
    index = (unsigned) virtAddr / PageSize;
    offset = (unsigned) virtAddr % PageSize;
    
    if (tlb == NULL) { // => page table, index a la tabla
	...
	entry = &pageTable[index];
        ...
        for(entry = NULL, i = 0; i < TLBSize; ++i){
    	    if (tlb[i].valid && (tlb[i].virtualPage == (int)index)){
				entry = &tlb[i]; // Lo encontro
				break;
			}
		}
		if (entry == NULL){ // No lo encontro
			 DEBUG('a', "*** no valid TLB entry found for this virtual page!\n");
			return PageFaultException; // Puede que pagina este en memoria
		}
    }

    if (entry->readOnly && writing){ // Error en tratar de escribir a un read only
    ...
    if (pageFrame >= NumPhysPages){ // Verificar error en tamano
